...
import { Delete20Filled, DocumentEdit20Filled } from "@fluentui/react-icons";
import { useRouter } from "next/navigation";
import { useState } from "react";
import { Item } from "./question-table";

export default function EditQuestionDialog({ item }: { item: Item }) {
  ...
  const [correctChoice, setCorrectChoice] = useState(item.correct_choice);
  const [selectedOptions, setSelectedOptions] = useState<string[]>(
    item.selected_options
  );
  const [orderItems, setOrderItems] = useState(item.order_items);
  const [fills, setFills] = useState(item.fills);
  const [file, setFile] = useState<string>();
  const [audio, setAudio] = useState<string>();
  const [video, setVideo] = useState<string>();
  const [categories, setCategories] = useState<string>(item.categories);
  const [difficulty, setDifficulty] = useState<string>(item.difficulty);
  ...
  const handleSubmit = async (e: any) => {
    ...
    console.log("/api/questions/" + item._id);
  ...
  return (
    <Dialog>
      <DialogTrigger disableButtonEnhancement>
        <Button icon={<DocumentEdit20Filled />}></Button>
      </DialogTrigger>
      <DialogSurface>
