...
import { createNewTest, updateTest } from "@/actions/tests";
...
export default function TestForm({ test }: { test: any }) {
  ...
  const [isOpened, setIsOpened] = useState<any>(
    test.state == "opened" ? true : false
  );

  const [formState, formAction] = useFormState(updateTest, initialState) as any;
