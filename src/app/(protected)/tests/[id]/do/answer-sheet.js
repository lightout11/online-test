...
export default function AnswerSheet({ resultInfo, testInfo, questions }) {
  const [answers, setAnswers] = useState({});
  const [tick, setTick] = useState(
    Date.now() +
      testInfo.duration * 60000 -
      (Date.now() - resultInfo.startDateTime.getTime())
  );
  ...
  return (
    <div>
      ...
      <Button
        ...
        onPress={() => {
          setLoading(true);
          submitResult(testInfo.id, answers);
        }}
        color="primary"
      >
        Nộp bài
